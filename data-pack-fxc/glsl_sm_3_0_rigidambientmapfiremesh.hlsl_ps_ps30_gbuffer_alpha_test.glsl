...
// Fragment Shader
//
// id: 534 - fxc/glsl_SM_3_0_rigidambientmapfiremesh.hlsl_PS_ps30_gbuffer_alpha_test.glsl
//
...
/*104*/	r0.w = uintBitsToFloat((floatBitsToInt(cb1.data[1].y) == int(1)) ? 0xffffffffu : 0x00000000u) /**/;
...
/*117*/	  r0.w = uintBitsToFloat((0.000000 < cb1.data[1].x) ? 0xffffffffu : 0x00000000u);
...
/*137*/	    r4.w = cb1.data[1].x * cb1.data[1].x;
/*138*/	    r4.w = r4.w * 1.250000;
/*139*/	    r4.w = min(r4.w, 1.000000);
/*140*/	    r5.w = saturate(cb1.data[1].x * cb1.data[1].x + -0.800000);
...
/*164*/	    r4.w = dot(vec2(cb1.data[0].xyxx), vec2(cb1.data[0].xyxx));
/*165*/	    r4.w = sqrt(r4.w);
...
/*180*/	  }
/*181*/	}
/*182*/	r0.w = vsOut_T7.y * -r1.x + r1.x;
/*183*/	r4.xyz = (r0.xyzx * vec4(5.000000, 5.000000, 5.000000, 0.000000) + vec4(0.200000, 0.200000, 0.200000, 0.000000)).xyz;
/*184*/	r4.xyz = (-r0.xyzx + r4.xyzx).xyz;
/*185*/	r0.xyz = saturate(vsOut_T7.yyyy * r4.xyzx + r0.xyzx).xyz;
/*186*/	r4.x = dot(vec3(r0.xyzx), vec3(vec4(0.212600, 0.715200, 0.072200, 0.000000)));
/*187*/	r4.xyz = (-r0.xyzx + r4.xxxx).xyz;
/*188*/	r0.xyz = (vsOut_T7.zzzz * r4.xyzx + r0.xyzx).xyz;
/*189*/	r4.x = dot(vec3(r2.xyzx), vec3(vec4(0.212600, 0.715200, 0.072200, 0.000000)));
/*190*/	r4.xyz = (-r2.xyzx + r4.xxxx).xyz;
/*191*/	r4.xyz = (vsOut_T7.zzzz * r4.xyzx + r2.xyzx).xyz;
/*192*/	color1.w = saturate(vsOut_T7.z * -r0.w + r0.w);
/*193*/	r0.w = uintBitsToFloat((cb2.data[4].y != 0.000000) ? 0xffffffffu : 0x00000000u);
/*194*/	r2.x = dot(vec3(r3.xyzx), vec3(r3.xyzx));
/*195*/	r2.x = inversesqrt(r2.x);
/*196*/	r2.xyz = (r2.xxxx * r3.xyzx).xyz;
/*197*/	color3.x = r2.w / r3.w;
/*198*/	r2.w = max(abs(r2.z), abs(r2.y));
/*199*/	r2.w = max(r2.w, abs(r2.x));
/*200*/	r2.xyz = (r2.xyzx / r2.wwww).xyz;
/*201*/	r1.xyz = (r2.xyzx * vec4(0.500000, 0.500000, 0.500000, 0.000000) + vec4(0.500000, 0.500000, 0.500000, 0.000000)).xyz;
/*202*/	color0.xyzw = saturate(r1.xyzw);
/*203*/	color1.xyz = saturate(sqrt(r0.xyzx)).xyz;
/*204*/	r4.w = uintBitsToFloat(floatBitsToUint(r0.w) & uint(0x3c008081u));
/*205*/	color2.xyzw = saturate(r4.xyzw);
/*206*/	color3.yzw = (vec4(0, 0, 0, 0)).yzw;
/*207*/	color4 = vsOut_P1;
/*208*/	return;
}
