...
// Fragment Shader
//
// id: 4717 - fxc/glsl_ENABLE_SHADOWS_SHADOWS_SOFTWARE_SM_3_0_rigidstandardfire.hlsl_PS_ps30_rigid_forward_lighting_gamma_correct_LDR.glsl
//
...
/*42*/	r4.x = 0;
/*43*/	r4.y = cb1.data[26].x * 0.050000;
...
/*89*/	r0.z = uintBitsToFloat((floatBitsToInt(cb4.data[1].y) == int(1)) ? 0xffffffffu : 0x00000000u) /**/;
...
/*102*/	  r0.z = uintBitsToFloat((0.000000 < cb4.data[1].x) ? 0xffffffffu : 0x00000000u);
...
/*122*/	    r1.w = cb4.data[1].x * cb4.data[1].x;
/*123*/	    r1.w = r1.w * 1.250000;
/*124*/	    r1.w = min(r1.w, 1.000000);
/*125*/	    r2.w = saturate(cb4.data[1].x * cb4.data[1].x + -0.800000);
...
/*149*/	    r1.w = dot(vec2(cb4.data[0].xyxx), vec2(cb4.data[0].xyxx));
/*150*/	    r1.w = sqrt(r1.w);
...
/*181*/	r4.xyz = (vsOut_T0.xyzx + -cb1.data[0].xyzx).xyz;
/*182*/	r1.w = dot(vec3(r4.xyzx), vec3(r4.xyzx));
/*183*/	r2.w = inversesqrt(r1.w);
/*184*/	r3.yzw = (r2.wwww * r4.xxyz).yzw;
/*185*/	r2.w = -r3.x + 1.000000;
/*186*/	r5.x = uintBitsToFloat((0.000000 < cb2.data[11].x) ? 0xffffffffu : 0x00000000u);
...
/*202*/	r9.xyz = (mix(cb2.data[2].xyzx, cb2.data[3].xyzx, greaterThan(floatBitsToUint(r8.xxxx), uvec4(0)))).xyz/**/;
/*203*/	r8.xyw = (mix(cb2.data[4].xyxz, cb2.data[5].xyxz, greaterThan(floatBitsToUint(r8.yyyy), uvec4(0)))).xyw/**/;
/*204*/	r10.xyz = (mix(cb2.data[6].xyzx, cb2.data[7].xyzx, greaterThan(floatBitsToUint(r8.zzzz), uvec4(0)))).xyz/**/;
...
/*214*/	if(floatBitsToUint(cb2.data[0].x) != 0u) {
...
/*293*/	  r2.w = dot(vec3(-cb2.data[0].yzwy), vec3(r3.yzwy));
...
/*310*/	r0.xyz = (r0.xyzx * cb2.data[1].xyzx).xyz;
/*311*/	r0.xyz = (r3.xyzx * r0.xyzx).xyz;
/*312*/	r0.xyz = (r6.xxxx * r0.xyzx).xyz;
/*313*/	r0.xyz = (r2.xyzx * cb2.data[1].xyzx + r0.xyzx).xyz;
/*314*/	r0.xyz = (r0.xyzx + r7.yzwy).xyz;
/*315*/	r2.xyz = (vsOut_T0.xyzx).xyz;
/*316*/	r2.w = 1.000000;
/*317*/	r1.x = dot(r2.xyzw, cb1.data[9].xyzw);
/*318*/	r1.y = dot(r2.xyzw, cb1.data[10].xyzw);
/*319*/	r0.w = dot(r2.xyzw, cb1.data[12].xyzw);
/*320*/	r1.xy = (r1.xyxx / r0.wwww).xy;
/*321*/	r2.xy = (uintBitsToFloat(uvec4(greaterThanEqual(r1.xyxx, cb1.data[34].xyxx)) * 0xffffffffu)).xy;
/*322*/	r1.xy = (uintBitsToFloat(uvec4(greaterThanEqual(cb1.data[34].zwzz, r1.xyxx)) * 0xffffffffu)).xy;
...
/*328*/	r0.w = sqrt(r1.w);
/*329*/	r1.x = saturate(cb3.data[0].w)/**/;
/*330*/	r1.x = -r1.x + 1.000000;
/*331*/	r1.x = r1.x * 8.000000 + -4.000000;
/*332*/	r1.y = saturate(cb3.data[1].x)/**/;
...
/*338*/	r0.w = exp2(r0.w);
/*339*/	r0.w = cb3.data[1].y / r0.w;
/*340*/	r0.w = saturate(-r0.w + cb3.data[1].y);
/*341*/	r1.x = -vsOut_T0.y + cb3.data[1].w;
/*342*/	r1.y = -cb3.data[1].z + cb3.data[1].w;
/*343*/	r1.x = r1.x + -cb3.data[1].z;
...
/*350*/	r1.y = sqrt(r1.y);
/*351*/	r1.z = max(cb3.data[2].z, 0.001000);
...
/*356*/	r1.y = r1.y * r1.y;
/*357*/	r1.y = r1.y * r1.z;
/*358*/	r0.w = cb3.data[2].x * r1.x + r0.w;
/*359*/	r1.xzw = (cb2.data[1].xxyz * cb3.data[0].xxyz).xzw;
/*360*/	r1.xzw = (r1.xxzw * vec4(1.500000, 0.000000, 1.500000, 1.500000)).xzw;
/*361*/	r1.xzw = (r1.xxzw * abs(cb2.data[0].zzzz)).xzw;
/*362*/	r2.xyz = (r6.xxxx * r1.xzwx).xyz;
/*363*/	r4.w = max(r4.y, 0.000000);
/*364*/	r3.xyzw = (texture(s_sky, r4.xwzx.stp)).xyzw;
/*365*/	r2.w = saturate(dot(vec2(r0.wwww), vec2(cb3.data[2].yyyy)));
...
/*370*/	r0.xyz = saturate(r0.wwww * r1.xyzx + r0.xyzx).xyz;
/*371*/	r0.xyz = (log2(r0.xyzx)).xyz;
/*372*/	r0.xyz = (r0.xyzx * cb0.data[0].zzzz).xyz;
/*373*/	r0.xyz = (exp2(r0.xyzx)).xyz;
/*374*/	color0.xyz = (min(r0.xyzx, vec4(1.000000, 1.000000, 1.000000, 0.000000))).xyz;
/*375*/	return;
}
