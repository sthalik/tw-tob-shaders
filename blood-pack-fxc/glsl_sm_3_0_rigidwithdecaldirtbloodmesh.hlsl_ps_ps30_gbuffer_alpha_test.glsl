...
// Fragment Shader
//
// id: 964 - fxc/glsl_SM_3_0_rigidwithdecaldirtbloodmesh.hlsl_PS_ps30_gbuffer_alpha_test.glsl
//
...
/*84*/	r5.xy = (vsOut_T1.xyxx * cb2.data[2].xyxx + vsOut_T6.yzyy).xy;
...
/*133*/	r0.w = uintBitsToFloat((floatBitsToInt(cb1.data[1].y) == int(1)) ? 0xffffffffu : 0x00000000u) /**/;
...
/*146*/	  r0.w = uintBitsToFloat((0.000000 < cb1.data[1].x) ? 0xffffffffu : 0x00000000u);
...
/*166*/	    r4.z = cb1.data[1].x * cb1.data[1].x;
/*167*/	    r4.z = r4.z * 1.250000;
/*168*/	    r4.z = min(r4.z, 1.000000);
/*169*/	    r5.w = saturate(cb1.data[1].x * cb1.data[1].x + -0.800000);
...
/*193*/	    r4.z = dot(vec2(cb1.data[0].xyxx), vec2(cb1.data[0].xyxx));
/*194*/	    r4.z = sqrt(r4.z);
...
/*209*/	  }
/*210*/	}
/*211*/	r0.w = vsOut_T7.y * -r1.x + r1.x;
/*212*/	r2.xyz = (r0.xyzx * vec4(5.000000, 5.000000, 5.000000, 0.000000) + vec4(0.200000, 0.200000, 0.200000, 0.000000)).xyz;
/*213*/	r2.xyz = (-r0.xyzx + r2.xyzx).xyz;
/*214*/	r0.xyz = saturate(vsOut_T7.yyyy * r2.xyzx + r0.xyzx).xyz;
/*215*/	r2.x = dot(vec3(r0.xyzx), vec3(vec4(0.212600, 0.715200, 0.072200, 0.000000)));
/*216*/	r2.xyz = (-r0.xyzx + r2.xxxx).xyz;
/*217*/	r0.xyz = (vsOut_T7.zzzz * r2.xyzx + r0.xyzx).xyz;
/*218*/	r2.x = dot(vec3(r4.xywx), vec3(vec4(0.212600, 0.715200, 0.072200, 0.000000)));
/*219*/	r2.xyz = (-r4.xywx + r2.xxxx).xyz;
/*220*/	r4.xyz = (vsOut_T7.zzzz * r2.xyzx + r4.xywx).xyz;
/*221*/	color1.w = saturate(vsOut_T7.z * -r0.w + r0.w);
/*222*/	r0.w = uintBitsToFloat((cb2.data[4].y != 0.000000) ? 0xffffffffu : 0x00000000u);
/*223*/	r2.x = dot(vec3(r3.xyzx), vec3(r3.xyzx));
/*224*/	r2.x = inversesqrt(r2.x);
/*225*/	r2.xyz = (r2.xxxx * r3.xyzx).xyz;
/*226*/	color3.x = r2.w / r3.w;
/*227*/	r2.w = max(abs(r2.z), abs(r2.y));
/*228*/	r2.w = max(r2.w, abs(r2.x));
/*229*/	r2.xyz = (r2.xyzx / r2.wwww).xyz;
/*230*/	r1.xyz = (r2.xyzx * vec4(0.500000, 0.500000, 0.500000, 0.000000) + vec4(0.500000, 0.500000, 0.500000, 0.000000)).xyz;
/*231*/	color0.xyzw = saturate(r1.xyzw);
/*232*/	color1.xyz = saturate(sqrt(r0.xyzx)).xyz;
/*233*/	r4.w = uintBitsToFloat(floatBitsToUint(r0.w) & uint(0x3c008081u));
/*234*/	color2.xyzw = saturate(r4.xyzw);
/*235*/	color3.yzw = (vec4(0, 0, 0, 0)).yzw;
/*236*/	color4 = vsOut_P1;
/*237*/	return;
}
