...
// Fragment Shader
//
// id: 2401 - fxc/glsl_SM_4_0_weightedwithdecalblood.hlsl_PS_ps30_gbuffer_skin_no_faction_colour_40.glsl
//
...
/*41*/	r3.xy = (vsOut_T1.xyxx * vec4(0.750000, 0.750000, 0.000000, 0.000000)).xy;
/*42*/	r3.xy = (r3.xyxx * cb1.data[2].xyxx + vsOut_T6.xyxx).xy;
/*43*/	r3.xyzw = (texture(s_decal_blood_map, r3.xyxx.st)).xyzw;
...
/*76*/	r3.xyzw = (texture(s_skin_mask_map, vsOut_T1.xyxx.st)).xyzw;
/*77*/	r0.w = vsOut_T7.x * -r2.x + r2.x;
/*78*/	r4.xyz = (r0.xyzx * vec4(5.000000, 5.000000, 5.000000, 0.000000) + vec4(0.200000, 0.200000, 0.200000, 0.000000)).xyz;
/*79*/	r4.xyz = (-r0.xyzx + r4.xyzx).xyz;
/*80*/	r0.xyz = saturate(vsOut_T7.xxxx * r4.xyzx + r0.xyzx).xyz;
/*81*/	r1.w = dot(vec3(r0.xyzx), vec3(vec4(0.212600, 0.715200, 0.072200, 0.000000)));
/*82*/	r4.xyz = (-r0.xyzx + r1.wwww).xyz;
/*83*/	color1.xyz = saturate(vsOut_T7.yyyy * r4.xyzx + r0.xyzx).xyz;
/*84*/	color1.w = saturate(vsOut_T7.y * -r0.w + r0.w);
/*85*/	r0.xyz = (vsOut_T0.xyzx).xyz;
/*86*/	r0.w = 1.000000;
/*87*/	r1.w = dot(r0.xyzw, cb0.data[11].xyzw);
/*88*/	r0.x = dot(r0.xyzw, cb0.data[12].xyzw);
/*89*/	color3.x = r1.w / r0.x;
/*90*/	r0.x = max(abs(r1.z), abs(r1.y));
/*91*/	r0.x = max(r0.x, abs(r1.x));
/*92*/	r0.xyz = (r1.xyzx / r0.xxxx).xyz;
/*93*/	r2.xzw = (r0.xxyz * vec4(0.500000, 0.000000, 0.500000, 0.500000) + vec4(0.500000, 0.000000, 0.500000, 0.500000)).xzw;
/*94*/	color0.xyzw = saturate(r2.xzwy);
/*95*/	color2.xyzw = r3.xyzz;
/*96*/	color3.yzw = (vec4(0, 0, 0, 0)).yzw;
/*97*/	color4 = floatBitsToUint(vsOut_T9.z);
/*98*/	return;
}
